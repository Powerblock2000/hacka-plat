...
func start_dialog() -> void:
	...
	await say_dialog("Press [E] (Keyboard) or [Y] (Xbox) to enter (Edit) mode                                              ")
	edit.show()
	await say_dialog("But be aware, you can only move objects a set amount of times. Check the botton of your sceen to see!")
	edits_remaining.show()
	await say_dialog("Dont worry, if you mess up you can press [R] (Keyboad) or [RB] (Xbox) to reset!")
	reset.show()
	await  say_dialog("Also, you can sprint by pressing [Shift] (Keyboard) or [X] (Xbox)")
	sprint.show()
	can_move = true
